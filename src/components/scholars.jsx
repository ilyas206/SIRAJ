...
export default function Scholars(){
    ...
    return(
        <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="w-full text-slate-200 p-4 mt-5">
            ...
            <div className="grid grid-cols-2 md:flex flex-wrap justify-center items-center gap-3 mt-3 mb-20">
                {
                    categories.map(category => {
                        return <button 
                                    key={category.id} 
                                    className={`flex items-center justify-center gap-3 ${selectedCategory === category.id ? 'bg-slate-500' : 'bg-slate-700 hover:bg-slate-600'} transition duration-300 font-light rounded-md p-2`}
