...
export default function Home(){
    ...
    return(
        <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="home relative w-full min-h-[calc(100vh-90px)] flex flex-col items-center justify-center text-slate-200 p-4">
            <h1 className="text-center md:w-10/12 text-6xl/tight md:text-8xl/tight mb-4 ">{handleTranslation('SIRAJ')} - {handleTranslation('Light of')} <span className="bg-slate-400 text-slate-900">{handleTranslation('Guidance')}</span></h1> 
            <p className="z-20">{handleTranslation('Built for the Ummah as a free guide everyday')}.</p>
            <img src="assets/images/moon.png" alt="Hilal" className="absolute opacity-70 left-4 bottom-7 md:bottom-1 md:w-56 w-36 h-36 md:h-56 lg:w-72 lg:h-72 " />
